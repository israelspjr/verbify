...
class Controller_Candidato_MeusCreditos extends Controller_Candidato_DefaultTemplate {
	...
	public function action_index()
	{
		...
		$logs = array();
		$logs[] = array("data" => $since, "descricao" => strtoupper(__("Saldo Anterior")), "valor" => $user->conta->getSaldo($dias));
		...
		$logs[] = array("data" => date('Y-m-d'), "descricao" => strtoupper(__('Saldo')), "valor" => $var["saldo"]);
		$var["logs"] = $logs;
		$var["dias"] = $dias;

		$this->template->title = __('meus créditos');
		$this->template->content = View::factory('candidato/meuscreditos', $var);
	...
	public function action_comprar(){
		...
		$this->template->title = __('meus créditos');
		$this->template->content = View::factory('candidato/comprar', $var);
	}
	...
	public function action_pagar(){
		try {
			...
			try {
				...
				if( ! $user->conta->loaded()){
					$user->conta->candidato_id = $user->id;
					$user->conta->save();
				}
				...
				$tel = substr(preg_replace("/[^0-9]/", "", $user->tel1), 2);
				$nome = substr($user->nome, 0, 50);
				$paymentRequest->setSender($nome, $user->email, substr($tel, 0, 2), substr($tel, 2));
				$paymentRequest->setRedirectUrl("http://www.profcerto.com.br");
