<h2 class="h2_subtitle">Alterar Vaga</h2>
<? echo (isset($success) ? '<p class="p_success">'.$success.'</p>' : ''); ?>
<? echo (Arr::get($errors, "vaga")<>"" ? '<p class="p_error">'.Arr::get($errors, "vaga").'</p>' : ''); ?>
<form method="post" id="frm_vaga">
	...
	<div>
		...
		<input type="checkbox" name="exibir_nome" value="1" id="exibir_nome" <?=(Arr::get($values, "exibir_nome")=="1" ? "checked" : "")?> />
...
<script>
	$(function(){
		...
		$("#dv_cidade").delegate("select[name='cidade']", "change", function(){
