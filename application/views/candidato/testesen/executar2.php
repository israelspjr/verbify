<h2 class="h2_subtitle"><?=$teste->getNome('en')?></h2>
<?=(isset($error) ? $error : '')?>
<? if($question->loaded()){ ?>
	<form method="post" id="frm_questao">
		...
		<div class="dv_questao">
			...
			<h3><? echo __('Pergunta'); ?> <?=$question->ordem?> / <?=$questions?> <?=($question->topico_en <> "" ? ": ".$question->topico_en : $question->topico)?></h3>
			<?=FormTestesEn::getHtmlCandidatoShowNextQuestion($question, $question->ordem, (isset($error) ? Arr::get($_POST,"resposta") : ''))?>
		</div>
