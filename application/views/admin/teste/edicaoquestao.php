		...
		<fieldset id="fs_addquestion">
			...
			<div id="tabs" style="margin-top: 10px;">
				...
				<div id="tabs-2">
					<div>
						<label>Tópico</label><br />
						<input name="topico_en" value="<?=$questao->topico_en?>" maxlength="300" class="iptresposta" />
						<span class="error"><?=Arr::get($errors, "topico_en")?></span>
					</div>
					<div>
						<label>Enunciado</label><br />
						<textarea class="questao" name="questao_en"><?=$questao->enunciado_en?></textarea>
						<span class="error"><?=Arr::get($errors, "enunciado_en")?></span>
					</div>
					<div id="dv_form_respostas_en">
						<?
						$i = 0;
						foreach($respostas as $row){
							$i++;
							echo FormTestesEn::getOpcoesCadastro($questao->tipo, $i, $row->texto_en, $row->valor);
						}
						?>
					</div>
				</div>
...
function addFormRespostaButton(){
	...
	$.post('<? echo URL::site("admin/cadastroforms/respostasen") ?>',
		{ posicao: i, tipo: $("#tipo").val() },
		function(data) {
			$("#dv_form_respostas_en").append(data);
		}
	);
}
</script>