...
<form id="frm-cadastro-talento" method="post">
	<fieldset>
		<legend><? echo __('Locais em que você está disponível para aulas'); ?></legend>
		<ul>
			<li>
				<label class="lbl-check">
					<input type="radio" name="tipo" value="ambos" <?=(($localidade->tipo_disponibilidade == "ambos" || $localidade->tipo_disponibilidade == "") ? "checked" : "") ?> /><span><? echo __('EAD e presencial'); ?></span>
				</label>
				<label class="lbl-check">
					<input type="radio" name="tipo" value="ead" <?=($localidade->tipo_disponibilidade == "ead" ? "checked" : "") ?>/><span><? echo __('só ferramentas de EAD'); ?>*</span>
				</label>
				<label class="lbl-check">
					<input type="radio" name="tipo" value="presencial" <?=($localidade->tipo_disponibilidade == "presencial" ? "checked" : "") ?>/><span><? echo __('só presencial'); ?></span>
				</label>
			</li>
			<li>
				<label class="lbl-check">
	<!--				<input type="radio" name="pais" checked value="30" <?=(($localidade->pais == "brasil" || $localidade->pais == "") ? "checked" : "") ?> /><span><? echo __('Brasil'); ?></span>-->
				</label>
				 <label class="lbl-check"><? echo __('País'); ?>
	<!--				<input type="radio" name="pais" value="outro" <?=((($localidade->pais <>"" && $localidade->pais <> "brasil") or Arr::get($values, 'pais') == "outro") ? "checked" : "") ?> /><span><? echo __('Outro país'); ?>. <? echo __('Qual'); ?>?</span>-->
        <select name="pais" class="ipt">
			...
			/*				if ($valor->id == 30) {
							...
							echo '<option value="'.$valor->id.'" '.(Arr::get($values, "nacionalidade") == $loca->pais ? 'selected' : '').'>'.$valor->name.'</option>';
//							}
						}
						?>
					</select>
