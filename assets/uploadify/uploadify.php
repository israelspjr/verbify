...
if(!preg_match($pattern, $_SERVER["HTTP_REFERER"])){
	echo 'Upload não autorizado'.$pattern.' '.$_SERVER["HTTP_REFERER"];
	exit;
}
*/
// Define a destination
$urlsite = (isset($_REQUEST["urlsite"]) ? $_REQUEST["urlsite"] : '');
$urlsite = preg_replace('/pt$|en$/i','',$urlsite);
$tutorial = $urlsite.'assets/img/tutorial_conversao.pdf';
$targetFolder = $urlsite.'uploads/videostmp/'; // Relative to the root
$targetPath = $_SERVER['DOCUMENT_ROOT'] . $targetFolder;
//$targetPath = "/home/proftobr/profcerto.com.br/www/uploads/videostmp/";
if (!empty($_FILES)) {
	...
	$fileTypesFlowPlayer = array('mp4', 'flv', 'mov'); // File extensions
	$fileTypes = array('avi', 'mp4', '3gp', 'psp', 'mpeg', 'mpegts', 'dvd', 'flv', 'svcd', 'vcd', 'vob', 'asf', 'mov', 'rm', 'mjpeg', 'mpeg2video', '3g2', 'mp3', 'ogg', 'wmv'); // File extensions
	$fileParts = pathinfo($_FILES['Filedata']['name']);

	if (in_array($fileParts['extension'], $fileTypesFlowPlayer)) {
		...
		try {
			...
			<div style="float: left; margin: 10px 20px; background: #fff; width:350px; padding: 10px;">
				...
				Caso o seu vídeo não seja reproduzido corretamente, clique em "CONVERTER" e aguarde um email notificando a conclusão.<br />
			</div>
			<div class="clear"></div>
			<input type="submit" name="gravar" class="uploadify-button" value="GRAVAR" style="width: 150px; height: 30px;" />
			<input type="submit" name="converter" class="uploadify-button" value="CONVERTER" style="width: 150px; height: 30px;" />
			<script>
			$(function(){
		...
		try {
			...
			echo '
			<input type="hidden" name="filename" value="'.$filename.'" />
			<input type="hidden" name="converter" value="CONVERTER" />
			<script>
			$(function(){
				$("#frm_upload").submit();
			});
		...
		$fileTypes = array('avi', 'mp4', '3gp', 'psp', 'mpeg', 'mpegts', 'dvd', 'flv', 'svcd', 'vcd', 'vob', 'asf', 'mov', 'rm', 'mjpeg', 'mpeg2video', '3g2', 'mp3', 'ogg', 'wmv'); // File extensions
		echo '<p class="p_error">O arquivo deve ser de uma das seguintes extensões: '.implode(',', $fileTypes).'</p>';
	}
}
